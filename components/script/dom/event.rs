...
impl Event {
    ...
    pub fn dispatch(
        ...
        warn!("In dispatch, type is {:?}", self.type_());
        ...
        let target_override = if legacy_target_override {
            warn!("In dispatch, applying target_override");
            ...
            warn!("In dispatch, not applying target_override");
            target
        };
        ...
        warn!("Path has length {:?}", event_path.len());
...
fn inner_invoke(
    ...
    for listener in listeners {
        ...
        // Step 2.10
        warn!("Calling a listener from inner_invoke");
        ...
        if event.stop_immediate.get() {
            warn!("leaving inner_invoke via stop_immediate");
            return found;
        }
    }
    warn!("leaving inner_invoke normally");
    // Step 3.
    found
}
